...
Four phases completed across GPU-server and Win-server. **~711GB reclaimed** total.
...
| 3 | GPU-server ~/models/ (GGUF purge + archived models + DeepSeek-OCR) | ~329GB |
| 4 | Win-server relator.IA/training/models/ + Downloads installers | ~158GB |

## Current Fleet State

### GPU-server (GPU workstation)
...
**soar-challenge on Win-server:** has untracked files not on GPU-server's copy: `eval/debug_memory.sh`, `eval/run_gptq_v2_eval.sh`, `scripts/gptq_quantize_sala_v2.py`, `scripts/run_gptq_v2.sh`, `gptqmodel_offload/` (4.67GB), a GPTQ log file. These should be synced to GPU-server's canonical copy.
...
- Docker shares with Win-server host — old image already removed.
...
### Phase 4 (continued) — Win-server Downloads naming + relocation

**~60 files** in `C:\Users\user\Downloads` need:
...
- `document.pdf`, `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.pdf` — unknown content
...
- `git status` on Win-server-WSL relator.IA clone
- Delete clone if clean
- Purge ~/.cache/ (50GB)

### Phase 6 — Macbook audit

- Run reconnaissance (same `du` survey as GPU-server/Win-server)
- Apply same decision framework

### Phase 7 — Conventions

Draft and deploy:
- **Model storage policy:** models live in `~/models/` (GPU-server) or project-specific dirs, never in HF cache for production use. GGUF format is not used — HF only.
...
5. **llama.cpp:** one copy per machine (GPU-server: ~/Development/llama.cpp, Win-server: C:\Users\user\Development\llama.cpp). Kept for GGUF converter utility.
6. **SOAR venvs moved** (not deleted) into soar-challenge/ — cheaper than rebuilding.

## Journal Location

`gpu-server:~/Development/maestro-mcp/docs/journal/fleet-cleanup-2026-03.md`
